...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    @staticmethod
    def get_white_pixel_ratio(pil_img, satThresh=0.93):
        # Ensure the image is in RGB format first
        if pil_img.mode != 'RGB':
        ...
        # Create mask for low saturation pixels (white/gray pixels)
        white_mask = hsv_img[:, :, 1] < satThresh

        # Calculate ratio
        ratio = np.sum(white_mask) / white_mask.size
        return ratio
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size = 256, step_size = 256,
        ...
        skip_log_path = os.path.join(save_path, f"{self.name}_skipped_patches.csv")
        ...
        if not os.path.exists(skip_log_path):
            with open(skip_log_path, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(["x", "y", "reason", "white_ratio"])
        ...
        #skipped_log_path = os.path.join(save_path, f"{self.name}_skipped_patches.txt")
        # iterable = [(coord, contour_holes, ref_patch_size[0], cont_check_fn, skip_log_path) for coord in coord_candidates]
        ...
        for coord in results:
            ...
            x, y = coord
            # Read the patch
            patch = self.wsi.read_region((x, y), patch_level, (patch_size, patch_size)).convert("RGB")
            white_ratio = WholeSlideImage.get_white_pixel_ratio(patch)

            if white_ratio >= 0.93:
                print(f"Skipped patch at {coord} due to white background: {white_ratio:.2f}")
                with open(skip_log_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([x, y, "white", round(white_ratio, 4)])
                continue
            
            # Otherwise, keep
            filtered_results.append(coord)

        results = np.array(filtered_results)
    ...
    @staticmethod
    def process_coord_candidate(coord, contour_holes, ref_patch_size, cont_check_fn, skip_log_path=None):
        if WholeSlideImage.isInContours(cont_check_fn, coord, contour_holes, ref_patch_size):
            ...
            if skip_log_path is not None:
                with open(skip_log_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([coord[0], coord[1], "outside_contour", "N/A"])
            return None
